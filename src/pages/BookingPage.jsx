...
export default function BookingPage() {
  ...
  return (
    <div style={{ background: '#0A0806', minHeight: '100vh', paddingBottom: 180 }}>
      <style>{`
      ...
      {/* Header */}
      <div style={{ background: 'rgba(10,8,6,0.97)', backdropFilter: 'blur(20px)', borderBottom: '1px solid rgba(255,255,255,0.05)', padding: '12px 0', position: 'sticky', top: 68, zIndex: 10 }}>
        <div className="container-app" style={{ maxWidth: 820, display: 'flex', alignItems: 'center', gap: 12, padding: '0 16px' }}>
          ...
          <div style={{ width: 36, height: 36, borderRadius: '50%', overflow: 'hidden', border: '1.5px solid rgba(201,150,90,0.3)', background: 'rgba(201,150,90,0.1)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            <p style={{ fontFamily: 'Cormorant Garamond, serif', fontSize: '1rem', fontWeight: 600, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{prof?.business_name}</p>
            <p style={{ fontSize: 11, color: 'rgba(247,242,234,0.35)' }}>📍 {prof?.city}</p>
          </div>
          <div style={{ textAlign: 'right', flexShrink: 0 }}>
            <p style={{ fontFamily: 'Cormorant Garamond, serif', fontSize: '1.3rem', color: '#C9965A', fontStyle: 'italic', lineHeight: 1 }}>{service?.price}€</p>
        ...
        <div className="booking-grid" style={{ display: 'grid', gridTemplateColumns: '1fr 300px', gap: 20, alignItems: 'start' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
            ...
            <div style={{ background: 'linear-gradient(135deg, rgba(201,150,90,0.08), rgba(201,150,90,0.03))', border: '1px solid rgba(201,150,90,0.2)', borderRadius: 14, padding: '14px 16px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 12 }}>
              <div>
                <p style={{ fontWeight: 600, fontSize: 14, marginBottom: 2 }}>{service?.name}</p>
                <p style={{ fontSize: 12, color: 'rgba(247,242,234,0.4)' }}>⏱ {service?.duration_minutes} min</p>
              </div>
              <p style={{ fontFamily: 'Cormorant Garamond, serif', fontSize: '1.8rem', color: '#C9965A', fontStyle: 'italic', lineHeight: 1, flexShrink: 0 }}>{service?.price}€</p>
            </div>

            {/* Step 1: Date */}
            <div style={{ background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.06)', borderRadius: 16, padding: '16px 14px 12px' }}>
              <p style={{ fontSize: 10, letterSpacing: '0.15em', textTransform: 'uppercase', color: 'rgba(247,242,234,0.35)', marginBottom: 12, display: 'flex', alignItems: 'center', gap: 8 }}>
                <span style={{ width: 18, height: 18, borderRadius: '50%', background: 'rgba(201,150,90,0.2)', display: 'inline-flex', alignItems: 'center', justifyContent: 'center', fontSize: 9, color: '#C9965A', fontWeight: 700, flexShrink: 0 }}>1</span>
                Selecciona el día
              </p>
                ...
                {days.map((day) => {
                  ...
                  return (
                    <button key={day.toISOString()} className="day-btn" onClick={() => { setSelectedDate(day); setSelectedSlot(null) }} style={{
                      flexShrink: 0, width: 46, height: 62,
                      display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center',
                      borderRadius: 12, border: `1px solid ${active ? 'transparent' : 'rgba(255,255,255,0.06)'}`,
                      ...
                      <span style={{ fontSize: 8, textTransform: 'uppercase', letterSpacing: '0.05em', fontWeight: active ? 700 : 400 }}>{format(day, 'EEE', { locale: es })}</span>
                      <span style={{ fontSize: 17, fontWeight: 700, lineHeight: 1.2 }}>{format(day, 'd')}</span>
                      <span style={{ fontSize: 8, opacity: 0.7 }}>{format(day, 'MMM', { locale: es })}</span>
                    </button>
                  )
                })}
              </div>
            ...
            <div style={{ background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.06)', borderRadius: 16, padding: '16px 14px' }}>
              <p style={{ fontSize: 10, letterSpacing: '0.15em', textTransform: 'uppercase', color: 'rgba(247,242,234,0.35)', marginBottom: 12, display: 'flex', alignItems: 'center', gap: 8 }}>
                <span style={{ width: 18, height: 18, borderRadius: '50%', background: 'rgba(201,150,90,0.2)', display: 'inline-flex', alignItems: 'center', justifyContent: 'center', fontSize: 9, color: '#C9965A', fontWeight: 700, flexShrink: 0 }}>2</span>
                Hora — {format(selectedDate, "d MMM", { locale: es })}
              </p>

              {loadingSlots ? (
                <div className="slots-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 6 }}>
                  {Array.from({ length: 8 }).map((_, i) => <div key={i} className="skeleton" style={{ height: 40, borderRadius: 8 }} />)}
                </div>
              ) : freeSlots.length === 0 ? (
                <div style={{ textAlign: 'center', padding: '20px 0' }}>
                  <p style={{ fontSize: '1.5rem', marginBottom: 6 }}>😔</p>
                  <p style={{ fontSize: 13, color: 'rgba(247,242,234,0.3)' }}>Sin disponibilidad · Prueba otro día</p>
                </div>
              ) : (
                <div className="slots-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 6 }}>
                  {freeSlots.map((slot) => {
            ...
            <div style={{ background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.06)', borderRadius: 16, padding: '16px 14px' }}>
              <p style={{ fontSize: 10, letterSpacing: '0.15em', textTransform: 'uppercase', color: 'rgba(247,242,234,0.35)', marginBottom: 12, display: 'flex', alignItems: 'center', gap: 8 }}>
                <span style={{ width: 18, height: 18, borderRadius: '50%', background: 'rgba(201,150,90,0.2)', display: 'inline-flex', alignItems: 'center', justifyContent: 'center', fontSize: 9, color: '#C9965A', fontWeight: 700, flexShrink: 0 }}>3</span>
              ...
              <textarea
                ...
                style={{ width: '100%', background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.07)', borderRadius: 10, padding: '10px 12px', color: '#F7F2EA', fontSize: 13, fontFamily: 'Outfit, sans-serif', resize: 'none', height: 76, boxSizing: 'border-box' }}
              />
            </div>
          </div>
      ...
      <div className="booking-summary-mobile" style={{
        ...
        padding: '12px 16px 32px', zIndex: 999,
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12 }}>
          ...
          <p style={{ fontFamily: 'Cormorant Garamond, serif', fontSize: '1.5rem', color: '#C9965A', fontStyle: 'italic', lineHeight: 1, flexShrink: 0 }}>{service?.price}€</p>
        </div>
        <button onClick={() => book()} disabled={!selectedSlot || isPending} style={{
        ...
        <p style={{ fontSize: 11, color: 'rgba(247,242,234,0.2)', textAlign: 'center' }}>Puedes cancelar hasta 24h antes</p>
      </div>
    </div>
  )
}