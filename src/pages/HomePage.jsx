...
import catPets      from '../assets/cat-pets.webp'
...
const CATEGORIES = [
  ...
  { img: catPets,      label: 'Mascotas',   value: 'pets' },
]
    ...
    <div style={{ background: '#F8F5F0', minHeight: '100vh', fontFamily: 'Outfit, sans-serif' }}>
      <style>{`
        * { box-sizing: border-box; }
        ...
        .cat-pill:hover .cat-img { transform: scale(1.08); }
        ...
        <div className="hide-sb cats-row" style={{ display: 'flex', gap: 16, overflowX: 'auto', padding: '0 20px 4px' }}>
          {CATEGORIES.map((cat, i) => (
            <button key={cat.value} className="cat-pill anim-fadeup" onClick={() => navigate(`/search?category=${cat.value}`)}
              ...
              <div className="cat-circle" style={{ width: 64, height: 64, borderRadius: '50%', border: '1.5px solid rgba(184,131,58,0.35)', display: 'grid', placeItems: 'center', background: 'linear-gradient(135deg, #FFFDF9, #FFF6E8)', overflow: 'hidden', transition: 'all 0.3s cubic-bezier(0.34,1.56,0.64,1)' }}>
                <img className="cat-img" src={cat.img} alt={cat.label} loading="lazy"
                  style={{ width: '78%', height: '78%', objectFit: 'contain', transition: 'transform 0.35s cubic-bezier(0.34,1.56,0.64,1)' }} />
              </div>
